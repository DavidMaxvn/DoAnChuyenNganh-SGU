                                        ...
                                        <select name="city_id" id="city_id">
                                            ...
                                            @foreach ($listCity as $city)
                                                <option data-shipping-fee="{{ $city->shipping_fee }}"
                                                    ...
                                                    Phí vận chuyển: {{ formatVnd($city->shipping_fee) }}</option>
                                            @endforeach
                                        </select>
                                        @error('address')
                                            <p class="alert alert-danger mt-2">{{ $message }}</p>
                            ...
                            <table class="table">
                                ...
                                <tfoot>
                                    ...
                                    <tr class="order-total total">
                                        ...
                                        <td><strong><span
                                                    data-total-base="{{ $total }}">{{ formatVnd($total) }}</span></strong>
                                        </td>
                                    </tr>
                                </tfoot>
                            </table>
                                ...
                                <div id="accordion">
                                    <div class="row">
                                        @foreach ($listCoupon as $coupon)
                                            <div class="col-4 p-3">
                                                <div class="badge badge-info coupon-select" data-id="{{ $coupon->id }}"
                                                    data-discount-max="{{ $coupon->discount_max }}"
                                                    @if ($coupon->type == 'price') data-discount="{{ $coupon->discount }}"
                                            @elseif($coupon->type == 'percent')
                                            data-discount="{{ ($total * $coupon->discount) / 100 }}" @endif
                                                    style="cursor: pointer;">
                                                    <h4>{{ $coupon->name }}</h4>
                                                    @if ($coupon->type == 'price')
                                                        <h6 style="white-space: break-spaces;">Giảm
                                                            {{ formatVnd($coupon->discount) }} </h6>
                                                    @elseif($coupon->type == 'percent')
                                                        <h6 style="white-space: break-spaces;">Giảm
                                                            {{ $coupon->discount }}% </h6>
                                                    @endif
...
@section('js')
    <script>
        $(document).ready(function() {
            ...
            // Hiện/ẩn nút VNPAY khi đổi phương thức
            var $paymentSelect = $('select[name="payment_type"][form="form-main"]');
            var $btnVnp = $('#btn-vnpay');
            var $form = $('#form-main');

            function toggleVnpButton() {
